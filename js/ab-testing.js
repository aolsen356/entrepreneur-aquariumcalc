/**
 * A/B Testing Framework for Aquarium Calculator
...
(function(window) {
  ...
  const STORAGE_KEY = 'aq_ab_tests';
  const EVENTS_KEY = 'aq_ab_events';
  ...
  function initTests() {
    ...
    ABTest.run('cta-color', ['green', 'orange', 'blue'], {
      ...
      'orange': function() {
        const ctas = document.querySelectorAll('button[type="submit"], .cta-button, a.bg-cyan-500, a.bg-sky-500');
        ctas.forEach(cta => {
      ...
      'results': function() {
        ...
        ctas.forEach(cta => {
          if (cta.textContent.toLowerCase().includes('calculate')) {
            cta.textContent = 'Get My Results';
          }
        });
      },
      'free': function() {
